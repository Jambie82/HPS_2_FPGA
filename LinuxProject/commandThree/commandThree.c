...
int main (int argc, char *argv[])
{
  ...
  while (1)
    ...
    case 'h':
      ...
      puts("commandThree - this command turns the LED sequencer On if it is Off, ");
      puts("  Off if it is On.  The LEDs are sequenced by the FPGA when a counter rolls");
      puts("  over to zero.This has no input input parameters.");
      ...
      puts("  > commandThree -t ");
      puts("  > Ok");
      puts("  This runs the command in terminal mode (interactive use) and toggles the");
      puts("  on/off condition of the LED sequencer.  The returned status here shows that the command");
      ...
      puts("  sent as four numeric strings).  The command starts or stops the LED state ");
      puts("  sequencer.  The returned data shows that the command executed correctly.");
  ...
  local_index = 0;
  data[1] = 0;  // param1
  data[2] = 0;  // param2, default is invalid argument so if no parameter passed we will not do anything
  data[3] = 0;  // param3
  if (optind < argc)
    {
	  if (verbose_flag)
...
//	int data[5];  // 8];
	...
	int i ;
		...
		N=4;
		// generate a sequence
		data[0] = 3;  // command number for commandThree
