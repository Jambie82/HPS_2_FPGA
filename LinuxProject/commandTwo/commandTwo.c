...
int main (int argc, char *argv[])
{
  ...
  while (1)
    ...
    case 'h':
      ...
      puts("commandTwo - this command toggles the setting on all four LEDs");
      puts("  This has no input input parameters.");
      ...
      puts("  on/off condition of the LEDs.  The returned status here shows that the command");
      ...
      puts("  sent as four numeric strings).  The command changes the condition of all four LEDs.");
...
//	int data[5];  // 8];
	...
	int i ;
		...
		N=4;
		// generate a sequence
		data[0] = 2;  // command number for commandTwo
